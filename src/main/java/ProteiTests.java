    ...
    public static void testFormValidationFieldsOverflow(WebDriver driver) {
        ...
        if (rowCount == 1) {
            ...
            if (!isEmailValid || !isNameValid) {
                System.out.println("Тест переполнения полей провален: запись с недопустимо длинными значениями добавлена в таблицу");
            } else {
                System.out.println("Тест переполнения полей прошел успешно: длинные значения были обрезаны перед добавлением в таблицу");
            }
        } else {
            System.out.println("Тест переполнения полей прошел успешно: запись с длинными значениями не была добавлена");
        }
    }
}
